...
export default async function SubCheckPage() {
  const [allIssues, mrdMd] = await Promise.all([
    fetchAllIssues(),
    fetchRepoFile('subcheck', 'docs/product/MRD.md'),
  ]);
    ...
    <div>
      <div className="mb-6 animate-fade-in">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-2xl font-bold text-foreground tracking-tight">SubCheck</h1>
          <SignalPill label="SETUP" tone="neutral" />
        </div>
        <p className="text-sm text-muted-foreground">Subscription audit & cancellation — iOS app via Apple IAP</p>
