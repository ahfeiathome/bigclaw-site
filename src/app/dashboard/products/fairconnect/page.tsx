...
export default async function FairConnectPage() {
  const [allIssues, mrdMd] = await Promise.all([
    fetchAllIssues(),
    fetchRepoFile('fairconnect', 'docs/product/MRD.md'),
  ]);

  const issues = allIssues.filter(i => i.repo === 'fairconnect');
  const p0 = issues.filter(i => i.labels.includes('P0'));

  return (
    <div>
      <div className="mb-6 animate-fade-in">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-2xl font-bold text-foreground tracking-tight">FairConnect</h1>
          <SignalPill label="SETUP" tone="neutral" />
        </div>
        <p className="text-sm text-muted-foreground">Fair & event social connector — iOS app via Apple IAP</p>
        ...
        <SectionCard title="Dependencies">
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-foreground">App Store Connect setup</span>
      ...
      {mrdMd && (
        <SectionCard title="MRD Status">
          <div className="flex items-center gap-2 text-sm">
            <StatusDot status="good" size="sm" />
            <span className="text-foreground">MRD document available</span>
          </div>
        </SectionCard>
      )}
    </div>
