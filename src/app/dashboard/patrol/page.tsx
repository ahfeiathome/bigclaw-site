...
function extractSection(content: string, heading: string): string {
  ...
  const nextSection = rest.search(/^## (?!.*\bMeta\b)/m);
  // Find next ## that isn't the current one
...
function KpiCard({
  ...
  return (
    <div className="border border-border rounded-lg p-4">
      <div
        className={`text-xs font-semibold ${color} uppercase tracking-wide mb-3`}
      >
        {title}
      </div>
...
export default async function PatrolPage() {
  ...
  const product = parseMarkdownTable(extractSection(content, 'Product'));
  const radar = parseMarkdownTable(extractSection(content, 'RADAR'));
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-medium">
              Felix Patrol — {meta['Timestamp'] || 'unknown'}
            </div>
            <div className="text-[10px] text-muted">
              Market: {meta['Market Phase'] || '?'} · Mode: {meta['Mode'] || '?'} ·
      ...
      {/* Alerts — full width at top if any */}
      <div className="mb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        ...
        <KpiCard title="🧪 Product" color="text-accent" rows={product} showTrend />
        <KpiCard title="📈 RADAR" color="text-cyan-400" rows={radar} showTrend />
      </div>
