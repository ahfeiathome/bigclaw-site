...
test.describe('Visual verification — dashboard', () => {
  ...
  test('finance page shows operational costs only', async ({ page }) => {
    ...
    // Wait for page content — h1 if data loaded, error div if GitHub API unavailable
    await page.waitForSelector('h1, [class*="text-center"]', { timeout: 20000 })

    // Screenshot comparison (page may show error state if the-firm repo is unreachable)
